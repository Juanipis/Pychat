...
def recivir_mensajes_servidor(clientsocket, client_name, msg_list):
		#------------------Recibir mesajes de chat--------------------------#
	while True:
		data = clientsocket.recv(1024)
		if not data: break
		data_msg = (client_name.decode('utf-8'), ': %s' %data.decode('utf-8'))
		msg_list.insert(tk.END, data_msg)


def enviar_mensaje_servidor(entry_fiel, clientsocket, msg_list, user):
	msg = entry_fiel.get()
...
def server(ventanaPrincipal, user, ventana):
	...
	thread = threading.Thread(name='recivir' ,target=recivir_mensajes_servidor, args=(clientsocket, client_name, msg_list,))	
	thread.start()

	#------------------Enviar mesajes de chat--------------------------#

	entry_fiel = tk.Entry(ventanaPrincipal)
	send_buttom = tk.Button(ventanaPrincipal, text="Enviar", command=lambda: enviar_mensaje_servidor(entry_fiel, clientsocket, msg_list, user))
	entry_fiel.pack()
	send_buttom.pack()
...
def ventana_seleccion(ventanaPrincipal, user,):
	...
	boton_cliente = tk.Button(ventana, text="cliente").grid(row=1, column=2)
